...
const SubscriptionsManager: React.FC = () => {
  ...
  const [planTypeFilter, setPlanTypeFilter] = useState<MealPlanType | 'all'>('all');
  ...
  const filteredSubscriptions = subscriptions.filter(subscription => {
    ...
    if (searchTerm) {
      ...
      const nameMatch = subscription.customerName?.toLowerCase().includes(searchLower);
      const emailMatch = subscription.customerEmail?.toLowerCase().includes(searchLower);
    ...
    <div style={containerStyle}>
      ...
      {analytics && (
        <div style={analyticsContainerStyle}>
          <div style={analyticsCardStyle}>
            ...
            <p style={{ fontSize: '2rem', fontWeight: '700', color: '#d62828', margin: '0.5rem 0' }}>
              {analytics.totalSubscriptions}
            </p>
          </div>
          <div style={analyticsCardStyle}>
            <h3>Active Subscriptions</h3>
            <p style={{ fontSize: '2rem', fontWeight: '700', color: '#1a8917', margin: '0.5rem 0' }}>
              {analytics.activeSubscriptions}
            </p>
          </div>
          <div style={analyticsCardStyle}>
            <h3>Paused</h3>
            <p style={{ fontSize: '2rem', fontWeight: '700', color: '#c5a60c', margin: '0.5rem 0' }}>
              {analytics.pausedSubscriptions}
            </p>
          </div>
          <div style={analyticsCardStyle}>
            <h3>Canceled</h3>
            <p style={{ fontSize: '2rem', fontWeight: '700', color: '#d12828', margin: '0.5rem 0' }}>
              {analytics.canceledSubscriptions}
            </p>
          </div>
          <div style={analyticsCardStyle}>
            <h3>Total Revenue</h3>
            <p style={{ fontSize: '2rem', fontWeight: '700', color: '#2b2b2b', margin: '0.5rem 0' }}>
      ...
      <div style={filterContainerStyle}>
        ...
        <select 
          ...
          onChange={(e) => setPlanTypeFilter(e.target.value as MealPlanType | 'all')}
        >
      ...
      {/* Subscriptions Table */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: '2rem' }}>Loading subscriptions...</div>
      ) : (
        <>
            ...
            <tbody>
              {filteredSubscriptions.length === 0 ? (
                ...
                filteredSubscriptions.map((subscription) => (
                  ...
                  >
                    <td style={tdStyle}>{subscription.id?.substring(0, 8)}...</td>
                    <td style={tdStyle}>
                      <div>{subscription.customerName}</div>
                      <div style={{ fontSize: '0.8rem', color: '#666' }}>{subscription.customerEmail}</div>
                    </td>
                    <td style={tdStyle}>
                      {subscription.planType === 'veg' ? 'Vegetarian' : 'Non-Vegetarian'} 
                      <div style={{ fontSize: '0.8rem', color: '#666' }}>{subscription.frequency}</div>
                    </td>
                    <td style={tdStyle}>{formatPrice(subscription.amount, subscription.currency)}</td>
                    <td style={tdStyle}>
                      <span style={getStatusBadgeStyle(subscription.status)}>
                        {subscription.status.charAt(0).toUpperCase() + subscription.status.slice(1)}
                      </span>
                    </td>
                    <td style={tdStyle}>{formatDate(subscription.startDate)}</td>
                    <td style={tdStyle}>{formatDate(subscription.nextBillingDate)}</td>
                    <td style={tdStyle}>
                      <button 
                        style={buttonStyle}
                        onClick={(e) => {
                          e.stopPropagation();
                          subscription.id && handleSubscriptionClick(subscription.id);
                        }}
                      >
                        View Details
                      </button>
